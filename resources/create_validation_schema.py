...
def convert_node_to_schema(node):
    ...
    for key, value in node.items():
        ...
        if isinstance(value, dict) and "type" in value:
            ...
            if compulsory is True or (isinstance(compulsory, str) and compulsory.lower() == "yes"):
                required.append(key)
        else:
...
def md_table_to_json_schema_template(md_filepath, json_schema_filepath):
    ...
    if len(lines) < 2:
        raise ValueError("The Markdown file does not contain enough lines to be a table.")
    ...
    for line in content_lines:
        ...
        if variable.startswith('['):
            ...
            if level == 1:
                ...
                if current_chapter is None:
                    raise ValueError("Found a subchapter before any chapter is defined.")
                ...
                if current_chapter is None or current_subchapter is None:
                    raise ValueError("Found a sub-subchapter without a parent chapter and subchapter.")
                current_subsubchapter = title
                nested_dict[current_chapter][current_subchapter].setdefault(current_subsubchapter, {})
            continue
        ...
        if current_chapter is None:
            ...
            nested_dict[current_chapter][current_subchapter][current_subsubchapter][variable] = param_dict

    # Convert the nested dictionary into a JSON Schema.
    # The top-level schema is an object whose properties are the top-level groups.
    json_schema = {
