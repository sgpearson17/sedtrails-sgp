...
class TestSedtrailsCLI:
    ...
    def test_load_config_default(self):
        ...
        assert (
            "Loading and validating configuration from 'sedtrails.yml'" in result.stdout
        )
    ...
    def test_load_config_custom(self):
        ...
        assert result.exit_code == 0
        assert (
            f"Loading and validating configuration from '{custom_config}'"
            in result.stdout
        )
        assert "Configuration validated successfully" in result.stdout
    ...
    def test_run_simulation_custom(self):
        ...
        assert "Running simulation..." in result.stdout
        assert (
            f"Simulation complete. Output saved to '{custom_output}'" in result.stdout
        )
    ...
    def test_analyze_custom(self):
        ...
        result = runner.invoke(
            app, ["analyze", "--input", custom_input, "--output", custom_output]
        )
        assert result.exit_code == 0
    ...
    def test_network_analysis_default(self):
        ...
        assert (
            "Network analysis complete. Results saved to 'sedtrails.nc'"
            in result.stdout
        )
    ...
    def test_network_analysis_custom(self):
        ...
        assert (
            f"Network analysis complete. Results saved to '{custom_output}'"
            in result.stdout
        )
