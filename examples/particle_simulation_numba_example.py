...
for step in range(1, NUM_STEPS + 1):
    ...
    new_x, new_y = calculator.update_particles(
        x0=np.array([particle_orig.x]),
        y0=np.array([particle_orig.y]),
        dt=TIMESTEP
    )
...
numba_calc = create_numba_particle_calculator(
    grid_x=flow_data['x'],
    grid_y=flow_data['y']
)
...
_ = numba_calc['update_particles'](
    np.array([particle_numba.x]),
    np.array([particle_numba.y]),
    flow_data['u'],
    flow_data['v'],
    TIMESTEP
)
...
position_diff = np.sqrt(
    (particle_orig.x - particle_numba.x)**2 + 
    (particle_orig.y - particle_numba.y)**2
)
print(f"\nResult comparison:")
...
trajectory_diffs = np.sqrt(
    (trajectory_orig_x - trajectory_numba_x)**2 + 
    (trajectory_orig_y - trajectory_numba_y)**2
)
...
fig, ax = plot_flow_field(
    flow_data=final_flow,
    title=f'Trajectory Comparison - {DURATION_HOURS:.2f} hours, {NUM_STEPS} steps',
    downsample=10
)

# Add original trajectory
ax.plot(trajectory_orig_x, trajectory_orig_y, '-', color='red', 
       linewidth=2, label='Original Implementation')
...
ax.plot(trajectory_numba_x, trajectory_numba_y, '-', color='blue', 
       linewidth=2, label='Numba Implementation')
