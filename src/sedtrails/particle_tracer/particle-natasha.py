...
class Particle(ABC):
    ...
    def Dstar_Tracer(self, S) -> float:
        ...
        S: user defined parameters for the simulation (config) or default values
        """
        return (S.g * (S.rhoParticle / S.rhoFluid - 1) / (S.visc_kin**2)) ** (
            1 / 3
        ) * S.dTracer
...
class Sand(Particle):
    ...
    def w_s_t(self, Dstar_Tracer, S) -> float:
        ...
        """
        return (S.visc_kin / S.dTracer) * (
            np.sqrt(10.36**2 + 1.049 * (Dstar_Tracer**3)) - 10.36
        )
    ...
    def theta_cr_Tracer(self, Dstar_Tracer) -> float:
        ...
        """
        return 0.3 / (1 + 1.2 * Dstar_Tracer) + 0.055 * (
            1 - np.exp(-0.020 * Dstar_Tracer)
        )
    ...
    def theta_cr_Background(self, Dstar_Background) -> float:
        ...
        """
        return 0.3 / (1 + 1.2 * Dstar_Background) + 0.055 * (
            1 - np.exp(-0.020 * Dstar_Background)
        )
    ...
    def U_bed(self, ustar_mean, theta_cr_Tracer_exp, theta_max_Tracer) -> float:
        ...
        return (
            10
            * ustar_mean
            * (1 - 0.7 * np.sqrt(theta_cr_Tracer_exp / theta_max_Tracer))
        )
