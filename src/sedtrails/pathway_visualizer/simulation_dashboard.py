...
class SimulationDashboard:
    ...
    def _setup_time_series_plots(self) -> None:
        ...
        self.lines['mixing_depth'], = self.axes['burial_depth'].plot([], [], 'gray', linestyle='--', label='Mixing Depth', linewidth=1)
    ...
    def _setup_progress_bar(self) -> None:
        ...
        self.progress_text_current = self.axes['progress'].text(50, 0.2, '', ha='center', va='bottom', fontsize=8, fontweight='bold')
    ...
    def _store_particle_data(self, particles: Dict[str, np.ndarray], 
                            current_time: float, timestep: float,
    ...
    def _update_flowfield_plot(self, flow_field: Dict[str, np.ndarray], bathymetry: np.ndarray) -> None:
        ...
        ax.quiver(x[skip], y[skip], flow_field['u'][skip], flow_field['v'][skip],
                 color='white', scale=10, width=0.004, alpha=0.8)  # Adjusted scale and width
    ...
    def _update_bathymetry_plot(self, flow_field: Dict[str, np.ndarray],
                               bathymetry: np.ndarray, particles: Dict[str, np.ndarray]) -> None:
        ...
        bathy_plot = ax.tricontourf(x, y, bathymetry, levels=20, 
                                   cmap=self.bathymetry_cmap,
                                   vmin=self.bathymetry_vmin, vmax=self.bathymetry_vmax)
        ...
        if len(particles['x']) > 0:
            ...
            ax.scatter(particles['x'], particles['y'], color='white', s=50, 
                      marker='o', edgecolors='black', linewidth=1, label='Current', zorder=5)
            
            # Initial positions (white crosses)
            if 'x_initial' in particles:
                ax.scatter(particles['x_initial'], particles['y_initial'], 
                          color='white', s=50, marker='x', linewidth=3, 
                          label='Initial', zorder=5)
                
                # Connect with lines
                for i in range(len(particles['x'])):
                    ax.plot([particles['x_initial'][i], particles['x'][i]],
                           [particles['y_initial'][i], particles['y'][i]],
                           'w-', alpha=0.7, linewidth=1, zorder=4)
    ...
    def save_dashboard(self, save_path: str) -> None:
...
def initialize_dashboard(reference_date: str = "1970-01-01", figsize: Tuple[float, float] = (16, 10)) -> SimulationDashboard:
...
def update_dashboard(dashboard: SimulationDashboard,
                    flow_field: Dict[str, np.ndarray],
                    bathymetry: np.ndarray,
                    particles: Dict[str, np.ndarray],
                    current_time: float,
                    ...
                    simulation_end_time: float = None) -> None:
    """Update dashboard with current simulation data."""
    dashboard.update_dashboard(flow_field, bathymetry, particles, 
                             current_time, timestep, plot_interval,
