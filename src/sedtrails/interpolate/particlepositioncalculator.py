...
class ParticlePositionCalculator:
    def __init__(self, grid_x: NDArray[np.float64], grid_y: NDArray[np.float64], grid_u: NDArray[np.float64], grid_v: NDArray[np.float64], igeo: int=0):
        """
        ...
        self.grid_v = grid_v
        self.igeo = igeo
        self.geofac = np.float64(6378137)  # conversion factor for geographic coordinates

        # Build the triangulation used for interpolation uisng Delaunay
        self.tri, self.trifinder = self.build_triangulation()

    def build_triangulation(self) -> Tuple[mtri.Triangulation, Callable[[float, float], int]]:
        """
    ...
    def _interpolate_field(self, field: NDArray, part_x: NDArray, part_y: NDArray) -> NDArray:
        """
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            w1 = ((y1 - y2) * (part_x[valid] - x2) + (x2 - x1) * (part_y[valid] - y2)) / den
            w2 = ((y2 - y0) * (part_x[valid] - x2) + (x0 - x2) * (part_y[valid] - y2)) / den
        w3 = 1.0 - w1 - w2
    ...
    @staticmethod
    def _interpolate_field_worker(args: Tuple[NDArray, NDArray, NDArray, NDArray, NDArray, NDArray]) -> NDArray:
        """
        ...
        den = (y1 - y2) * (x0 - x2) + (x2 - x1) * (y0 - y2)
        with np.errstate(divide='ignore', invalid='ignore'):
            w1 = ((y1 - y2) * (part_x_chunk[valid] - x2) + (x2 - x1) * (part_y_chunk[valid] - y2)) / den
            w2 = ((y2 - y0) * (part_x_chunk[valid] - x2) + (x0 - x2) * (part_y_chunk[valid] - y2)) / den
        w3 = 1.0 - w1 - w2
        interp_vals[valid] = w1 * field[v0] + w2 * field[v1] + w3 * field[v2]
        return interp_vals

    def parallel_interpolate_field(self, field: NDArray, part_x: NDArray, part_y: NDArray, num_workers: int) -> NDArray:
        """
        ...
        args_list = []
        for idx in indices:
            args = (self.grid_x, self.grid_y, field, self.tri.triangles,
                    part_x[idx], part_y[idx])
            args_list.append(args)
        with mp.Pool(num_workers) as pool:
            results = pool.map(ParticlePositionCalculator._interpolate_field_worker, args_list)
        return np.concatenate(results)

    def interpolate_field(self, field: NDArray, part_x: NDArray, part_y: NDArray, parallel: bool=False, num_workers: int=None) -> NDArray:
        """
    ...
    def update_particles(self, x0:NDArray, y0:NDArray, dt:np.float64, rndfac:np.float64, parallel:bool=False, num_workers:int=None) -> Tuple[NDArray, NDArray, NDArray,NDArray]:
        """
        ...
        def get_interp(field, part_x, part_y):
            return self.interpolate_field(field, part_x, part_y,
